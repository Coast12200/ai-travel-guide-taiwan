...
export async function generateItinerary(type, options = {}) {
    ...
    const chosenStyle = options && options.style ? options.style :
        (document.getElementById('itineraryStyleSelect') ? document.getElementById('itineraryStyleSelect').value : '');

    const budgetLevel = options && options.budgetLevel ? options.budgetLevel :
        (document.getElementById('budgetLevelSelect') ? document.getElementById('budgetLevelSelect').value : 'comfort');

    const transportPref = options && options.transportPref ? options.transportPref :
        (document.getElementById('transportModeSelect') ? document.getElementById('transportModeSelect').value : 'driving');
    ...
    // Safely hide transport container if it exists
    ...
    try {
        ...
        // 保存生成的版本
        if (window.saveItineraryVersion) {
            try {
                window.saveItineraryVersion('generate');
                console.log('✅ Itinerary version saved (generate)');
            } catch (error) {
                console.error('❌ Failed to save itinerary version:', error);
            }
        }
        ...
        if (itineraryActions) {
            ...
            console.log('✅ Itinerary action buttons displayed');
        }
...
export async function generateTransportSuggestions() {
    ...
    if (!container) {
        ...
        if (suggestionWrapper) {
            ...
            container.id = 'transportContent';
            container.className = 'ai-bubble';
            container.style.marginTop = '20px';
            suggestionWrapper.appendChild(container);
        } else {
            // 如果連 suggestionContentWrapper 都不存在，使用 suggestionContent
            container = document.getElementById('suggestionContent');
        }
    }
...
- strengths: 三大優勢陣列（純文字，不含符號）`;
    ...
    try {
        ...
        if (window.saveItineraryVersion) {
            try {
                window.saveItineraryVersion('optimize');
                console.log('✅ Optimized version saved');
            } catch (error) {
                console.error('❌ Failed to save optimized version:', error);
            }
        }
...
export async function generateFeedbackItinerary(feedback = '', feedbackType = '') {
    ...
    try {
        ...
        if (window.saveItineraryVersion) {
            try {
                window.saveItineraryVersion('feedback');
                console.log('✅ Feedback version saved');
            } catch (error) {
                console.error('❌ Failed to save feedback version:', error);
            }
        }
    } catch (err) {
...
export async function generateSouvenirList() {
    ...
    try {
        ...
        if (!sampleLocations) {
            return showError('請先選擇景點或生成行程以獲取伴手禮推薦', container);
        }
...
export function downloadEnhancedCSV() {
    try {
        ...
        appState.currentItineraryLocations.forEach((loc, index) => {
            const dest = destinationsByCountry['taiwan']?.destinations?.find(d => d.name === loc);
            if (!dest) return;
...
function calculateDistance(loc1Name, loc2Name) {
    const dest1 = destinationsByCountry['taiwan']?.destinations?.find(d => d.name === loc1Name);
    const dest2 = destinationsByCountry['taiwan']?.destinations?.find(d => d.name === loc2Name);
