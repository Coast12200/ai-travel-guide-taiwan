...
import { callGeminiAPI, callGeminiAPIWithSchema, callTtsAPI, fetchTdxData, fetchTdxTraffic } from './api.js';
...
export async function generateItinerary(type) {
    ...
    // [新增] 獲取日期和天氣資訊
    const selectedDate = appState.travelDate;
    let weatherInfo = '';
    
    if (appState.isCwaApiVerified && selectedDate) {
        // 嘗試獲取全台灣的天氣總覽或主要城市的天氣作為參考
        const taipeiForecasts = appState.weatherForecasts['臺北市'];
        if (taipeiForecasts) {
            const targetForecast = taipeiForecasts.find(f => 
                selectedDate >= f.startTime.split(' ')[0] && selectedDate <= f.endTime.split(' ')[0]
            );
            if (targetForecast) {
                // 將天氣資訊加入到 AI 規劃提示詞中
                weatherInfo = `，請特別考慮當日天氣：**${selectedDate}**，臺北市預計天氣為「${targetForecast.weatherDesc}」，氣溫約 ${targetForecast.minT}°C ~ ${targetForecast.maxT}°C，降雨機率 ${targetForecast.pop}%。請根據此天氣資訊優化行程，例如：若下雨多安排室內景點。`;
            }
        }
    }

    if (appState.currentCountry !== 'taiwan' && type !== 'random') {
        showError('多日規劃和天氣規劃功能目前僅支援台灣地區。', container);
        return;
    }
    ...
    try {
        ...
        const prompt = createItineraryPrompt(type, allAttractions, prefs, weatherInfo);
...
function createItineraryPrompt(type, allAttractions, prefs, weatherInfo) {
    ...
    const basePrompt = (type === 'multi-day')
     ...
     return `${basePrompt} 規則：
        1. ${weatherConstraint} ${userPrefs}${weatherInfo}
...
// 新增交通與票務查詢功能
export async function queryTransportInfo() {
    const container = document.getElementById('transportResults');
    const button = document.getElementById('queryTransportBtn');
    
    if (!appState.isTdxApiVerified) {
        return showError('此功能需要驗證 TDX API', container);
    }
    
    if (!appState.currentItineraryLocations || appState.currentItineraryLocations.length < 2) {
        return showError('請先生成包含至少兩個地點的行程', container);
    }
    
    // 顯示載入狀態
    button.innerHTML = '<div class="spinner" style="width: 16px; height: 16px; margin: auto;"></div>';
    button.disabled = true;
    container.innerHTML = '<div class="loading"><div class="spinner"></div>正在查詢即時交通資訊...</div>';
    ...
    try {
        ...
        for (let i = 0; i < locations.length - 1; i++) {
            ...
            try {
                // 查詢不同交通方式的資訊
                const [busResult, railResult, mrtResult] = await Promise.allSettled([
                    fetchTdxTraffic(from, to, 'Bus'),
                    fetchTdxTraffic(from, to, 'Rail'),
                    fetchTdxTraffic(from, to, 'MRT')
                ]);
                
                const routeInfo = {
                    from,
                    to,
                    bus: busResult.status === 'fulfilled' ? busResult.value : null,
                    rail: railResult.status === 'fulfilled' ? railResult.value : null,
                    mrt: mrtResult.status === 'fulfilled' ? mrtResult.value : null
                };
                
                allResults.push(routeInfo);
            } catch (error) {
                console.warn(`查詢 ${from} 到 ${to} 的交通資訊失敗:`, error);
            }
        }
        
        // 渲染結果
        renderTransportResults(allResults, container);
        
    } catch (error) {
        showError(`交通查詢失敗: ${error.message}`, container, queryTransportInfo);
    } finally {
        button.innerHTML = '🔍 查詢即時班次';
        button.disabled = false;
    }
}

export async function queryTicketInfo() {
    const container = document.getElementById('ticketSuggestions');
    const button = document.getElementById('queryTicketsBtn');
    
    if (!appState.isGeminiApiVerified) {
        return showError('此功能需要驗證 Gemini API', container);
    }
    
    if (!appState.currentItineraryLocations || appState.currentItineraryLocations.length === 0) {
        return showError('請先生成行程', container);
    }
    
    // 顯示載入狀態
    button.innerHTML = '<div class="spinner" style="width: 16px; height: 16px; margin: auto;"></div>';
    button.disabled = true;
    container.innerHTML = '<div class="loading"><div class="spinner"></div>AI 正在查詢票務資訊...</div>';
    
    try {
        const locations = appState.currentItineraryLocations.join(' -> ');
        const prompt = `你是一位專業的旅遊票務顧問。根據以下行程：${locations}

請為這個行程推薦相關的票務資訊，包括：
1. 交通票券（如高鐵票、台鐵票、捷運一日券等）
2. 景點門票（如博物館、遊樂園、展覽等）
3. 套票優惠（如交通+景點套票）

請用繁體中文，以 Markdown 格式呈現，並提供：
- 票券名稱
- 大致價格範圍
- 購買建議（官方網站或推薦平台）
- 使用注意事項
...
請確保資訊實用且符合台灣旅遊現況。`;
        ...
        const result = await callGeminiAPI(prompt);
        container.innerHTML = formatAsTimeline(result);
        
    } catch (error) {
        showError(`票務查詢失敗: ${error.message}`, container, queryTicketInfo);
    } finally {
        button.innerHTML = '🎫 查詢票務資訊';
        button.disabled = false;
    }
}

function renderTransportResults(results, container) {
    if (results.length === 0) {
        container.innerHTML = '<p>無法獲取交通資訊，請稍後再試。</p>';
        return;
    }
    
    let html = '<h4>🚗 即時交通資訊</h4>';
    
    results.forEach((route, index) => {
        html += `
            <div class="transport-route" style="margin-bottom: 20px; padding: 15px; border: 1px solid var(--border-color); border-radius: 8px;">
                <h5>📍 ${route.from} → ${route.to}</h5>
        `;
        
        // 公車資訊
        if (route.bus && route.bus.length > 0) {
            html += '<div class="transport-mode"><strong>🚌 公車路線：</strong><ul>';
            route.bus.slice(0, 3).forEach(bus => {
                html += `<li>${bus.RouteName?.Zh_tw || '未知路線'} (${bus.RouteID || 'N/A'})</li>`;
            });
            html += '</ul></div>';
        }
        
        // 台鐵資訊
        if (route.rail && route.rail.length > 0) {
            html += '<div class="transport-mode"><strong>🚂 台鐵車站：</strong><ul>';
            route.rail.slice(0, 3).forEach(station => {
                html += `<li>${station.StationName?.Zh_tw || '未知車站'}</li>`;
            });
            html += '</ul></div>';
        }
        
        // 捷運資訊
        if (route.mrt && route.mrt.length > 0) {
            html += '<div class="transport-mode"><strong>🚇 捷運車站：</strong><ul>';
            route.mrt.slice(0, 3).forEach(station => {
                html += `<li>${station.StationName?.Zh_tw || '未知車站'}</li>`;
            });
            html += '</ul></div>';
        }
