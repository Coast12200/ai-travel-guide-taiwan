...
export function renderCwaData(data, locationName) {
    ...
    if (data.warnings && data.warnings.length > 0) {
        ...
        data.warnings.forEach(alert => {
            ...
            li.innerHTML = `<strong>${headline}</strong>: ${description}`;
    ...
    if (data.forecast && data.forecast.weatherElement) {
        ...
        timeData.forEach((timeElement, idx) => {
            ...
            card.className = 'itinerary-day-card cwa-forecast-card';
            card.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <h4 style="margin:0;">${dateStr}</h4>
                    <span style="font-size:0.85rem; color:var(--muted);">${date.getDate() === new Date().getDate() ? '今天' : ''}</span>
                </div>
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; font-size:0.95rem;">
                    <div><strong>天氣：</strong>${weatherDesc}</div>
                    <div><strong>溫度：</strong>${minTemp} ~ ${maxTemp}</div>
                    <div><strong>降雨機率：</strong>${rainProb}</div>
                    <div><strong>舒適度：</strong>${comfort}</div>
                </div>
            `;
        ...
        scenicSpots.slice(0, 10).forEach(spot => {
            ...
            card.innerHTML = `
                <div style="padding: 12px;">
                    <h4 style="margin: 0 0 8px 0; color: var(--dark-text);">${spot.ScenicSpotName || 'N/A'}</h4>
                    <small style="color: var(--muted); display: block; margin-bottom: 8px;">${spot.Address || '無地址資訊'}</small>
                    <p style="font-size: 0.9rem; line-height: 1.5; margin: 8px 0;">${(spot.Description || '').substring(0, 100)}...</p>
                    <div style="margin-top: 10px; display: flex; gap: 15px; font-size: 0.9rem;">
                        <span style="color: var(--sun-orange);">🌟 ${spot.Grade || 'N/A'}</span>
                        <span>📞 ${spot.Phone || '無電話'}</span>
                    </div>
                </div>
            `;
...
export function setupEventListeners() {
    ...
    if (downloadMenuBtn && downloadBottomSheet) {
...
function renderDestinationsAccordion(destinations) {
    ...
    destinations.forEach(dest => {
        ...
        if (groupedByRegion[region]) {
            groupedByRegion[region].push(dest);
        } else {
            if (!groupedByRegion['other']) groupedByRegion['other'] = [];
...
function createCardElement(dest) {
    ...
    // Try to fetch realtime details (TDX) and populate status (async, fire-and-forget)
    (async () => {
        try {
            if (!appState.isTdxApiVerified) return;
            // if id looks offline (prefixed), skip
            if (String(dest.id).startsWith('offline')) return;
            const details = await fetchTdxScenicSpotDetails(dest.id);
            const el = document.getElementById(`status-${dest.id}`);
            if (!el) return;
            if (!details) {
                el.textContent = '無即時資料';
                return;
            }

            // Display open time and phone if available
            const parts = [];
            if (details.openTime) parts.push(`營業時間：${details.openTime}`);
            if (details.phone) parts.push(`電話：${details.phone}`);
            // crowd data not available from TDX; leave placeholder
            parts.push('人潮：無資料');

            // determine open/closed heuristically if openTime contains a time range
            const nowStatus = (() => {
                try {
                    const ot = details.openTime || '';
                    const regex = /(\d{1,2}:?\d{0,2})\s*-\s*(\d{1,2}:?\d{0,2})/;
                    const m = ot.match(regex);
                    if (m) {
                        const toMinutes = s => {
                            const clean = s.replace(/[^0-9:]/g, '');
                            const [h, mm] = clean.split(':').map(Number);
                            return (h || 0) * 60 + (mm || 0);
                        };
                        ...
                        if (start <= end) return (now >= start && now <= end) ? '狀態：營業中' : '狀態：休息中';
                        // overnight ranges
                        return (now >= start || now <= end) ? '狀態：營業中' : '狀態：休息中';
                    }
                } catch (e) { /* ignore parsing errors */ }
                return null;
            })();

            if (nowStatus) parts.unshift(nowStatus);

            el.textContent = parts.join(' ｜ ');
        } catch (err) {
            const el = document.getElementById(`status-${dest.id}`);
            if (el) el.textContent = '即時資料載入失敗';
            console.warn('enrich dest status failed', err);
        }
    })();
...
function handleSearch(event) {
    const searchTerm = (event && event.target && event.target.value || '').toLowerCase();
    const cards = Array.from(document.querySelectorAll('.destination-card'));

    // Toggle card visibility based on name/city match
    cards.forEach(card => {
        const name = (card.dataset.name || '').toLowerCase();
        const city = (card.dataset.city || '').toLowerCase();
        const isVisible = searchTerm === '' || name.includes(searchTerm) || city.includes(searchTerm);
        card.classList.toggle('hidden', !isVisible);
    });

    // For each region accordion, show/hide and optionally expand if it has matches
    const regions = Array.from(document.querySelectorAll('.region-accordion-item'));
    regions.forEach(regionItem => {
        const content = regionItem.querySelector('.region-accordion-content');
        const anyVisible = Array.from(regionItem.querySelectorAll('.destination-card')).some(c => !c.classList.contains('hidden'));
        // hide the entire region if no matches
        regionItem.style.display = anyVisible ? '' : 'none';
        // if there is a search term and matches, expand the region for visibility
        if (searchTerm && anyVisible) {
            regionItem.classList.add('active');
            if (content) content.style.maxHeight = content.scrollHeight + 'px';
            const icon = regionItem.querySelector('.accordion-icon'); if (icon) icon.textContent = '−';
        } else {
            // collapse region when search cleared
            if (!searchTerm) {
...
export function formatAsTimeline(markdownText) {
    ...
    for (const line of lines) {
        if (line.startsWith('###') || line.startsWith('##') || line.startsWith('**')) {
            ...
            html += `<li style="display: flex; align-items: center; gap: 8px;"><span style="flex: 1;">${itemText}</span>${ttsBtn}</li>`;
        } else if (line.trim()) {
