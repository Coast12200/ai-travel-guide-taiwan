...
import { appState, translations } from './state.js';
...
export function formatTime(timestamp) {
    ...
    // 小於 1 分鐘
    if (diff < 60000) {
        return translations.time_just_now[lang];
    }
    // 小於 1 小時
    if (diff < 3600000) {
        const minutes = Math.floor(diff / 60000);
        return `${minutes} ${translations.time_mins_ago[lang]}`;
    }
    // 小於 24 小時
    if (diff < 86400000) {
        const hours = Math.floor(diff / 3600000);
        return `${hours} ${translations.time_hours_ago[lang]}`;
    }
    // 小於 7 天
    if (diff < 604800000) {
        const days = Math.floor(diff / 86400000);
        return `${days} ${translations.time_days_ago[lang]}`;
    }
...
export function getTypeLabel(type) {
    const lang = appState.currentLanguage || 'zh';
    const key = `version_type_${type}`; // e.g. version_type_generate

    if (translations[key] && translations[key][lang]) {
        return translations[key][lang];
    }

    return lang === 'en' ? '📝 Other' : '📝 其他';
}
