...
export async function verifyCwaApi() {
    ...
    try {
        // [修改] 調整 URL 以取得包含縣市的 36 小時天氣預報 (F-C0032-001)
        ...
        // [修改] 將原始資料暫存到 appState
        appState.cwaRawData = data; 
...
export async function fetchTdxTraffic(departureStop, arrivalStop, transportType) {
    if (!appState.tdxAccessToken) {
        await getTdxAccessToken();
    }
    
    if (!appState.tdxAccessToken) {
        throw new Error('TDX API 未驗證，請先驗證 TDX API');
    }
    ...
    switch (transportType) {
        case 'Bus':
            // 查詢公車路線資訊
            apiUrl = `https://tdx.transportdata.tw/api/basic/v2/Bus/Route/City/Taipei?$filter=contains(RouteName/Zh_tw,'${encodeURIComponent(departureStop)}') or contains(RouteName/Zh_tw,'${encodeURIComponent(arrivalStop)}')&$format=JSON`;
            break;
        case 'Rail':
            // 查詢台鐵車站資訊
            apiUrl = `https://tdx.transportdata.tw/api/basic/v2/Rail/TRA/Station?$format=JSON`;
            break;
        case 'HSR':
            // 查詢高鐵車站資訊
            apiUrl = `https://tdx.transportdata.tw/api/basic/v2/Rail/THSR/Station?$format=JSON`;
            break;
        case 'MRT':
            // 查詢捷運車站資訊
            apiUrl = `https://tdx.transportdata.tw/api/basic/v2/Rail/Metro/Station?$format=JSON`;
            break;
        default:
            throw new Error(`不支援的交通類型: ${transportType}`);
    }
    
    try {
        const response = await fetch(apiUrl, {
            headers: {
                'Authorization': `Bearer ${appState.tdxAccessToken}`,
        ...
        if (response.status === 401) {
            ...
            if (!retryResponse.ok) {
                throw new Error(`TDX API 請求失敗，狀態碼: ${retryResponse.status}`);
            }
            return await retryResponse.json();
        }
        
        if (!response.ok) {
            throw new Error(`TDX API 請求失敗，狀態碼: ${response.status}`);
        }
        
        return await response.json();
    } catch (error) {
        console.error('TDX Traffic fetch failed:', error);
        throw new Error(`TDX 交通查詢失敗: ${error.message}`);
    }
}
