...
import { 
    ...
    queryTransportInfo, queryTicketInfo
} from './itinerary.js';
...
export function setupEventListeners() {
    ...
    document.getElementById('travelDate').addEventListener('change', (e) => {
        appState.travelDate = e.target.value;
        // 日期改變時，重新整理天氣顯示
        updateWeatherDisplays(); 
    });
}
...
function createCardHTML(dest) {
    ...
    // 呼叫新的函式來獲取天氣 HTML
    const weatherHtml = getDestinationWeather(dest);
...
function selectDestination(destinationId) {
    ...
    // 顯示交通與票務面板（如果已驗證 TDX API）
    if (appState.isTdxApiVerified) {
        document.getElementById('transportAndTicketsPanel').classList.remove('hidden');
    }
}

export function updateWeatherDisplays() {
    if (!appState.isCwaApiVerified || !appState.cwaRawData) {
        return; 
    }

    // 1. 將原始 CWA 資料處理並存入 appState
    appState.weatherForecasts = processCwaData(appState.cwaRawData);

    // 2. 重新渲染景點卡片以顯示天氣資訊
    if (appState.currentCountry === 'taiwan' && destinationsByCountry.taiwan.destinations.length > 0) {
        renderDestinationsAccordion(destinationsByCountry.taiwan.destinations);
    }
}

/**
 * 解析 CWA 36 小時預報資料，將其整理為 { [locationName]: forecastData } 結構。
 */
function processCwaData(rawData) {
    const forecasts = {};
    if (!rawData || rawData.success !== 'true') return forecasts;

    const locationData = rawData.records.location;
    ...
    locationData.forEach(loc => {
        ...
        // 提取天氣現象 (Wx)、最低溫 (MinT)、最高溫 (MaxT) 和降雨機率 (PoP)
        weatherElements.forEach(element => {
            data[element.elementName] = element.time.map(timeSlot => ({
                startTime: timeSlot.startTime,
                endTime: timeSlot.endTime,
                value: timeSlot.elementValue[0].value,
                desc: timeSlot.elementValue[0].measures,
            }));
        });
        
        // 整理成易於查詢的結構
        const processed = data.Wx.map((wxTime, index) => {
            return {
                startTime: wxTime.startTime,
                endTime: wxTime.endTime,
                weatherDesc: wxTime.value,
                // Wx, MinT, MaxT, PoP 的時間軸是同步的
                minT: data.MinT[index].value,
                maxT: data.MaxT[index].value,
                pop: data.PoP[index].value,
            };
        });

        forecasts[locationName] = processed;
    });
...
/**
 * 根據景點名稱和選定日期，從 appState 獲取並格式化天氣資訊。
 */
function getDestinationWeather(destination) {
    if (!appState.isCwaApiVerified || !appState.travelDate) return '';
    
    const locationName = destination.city || destination.name; 
    
    // 嘗試從 appState.weatherForecasts 找到匹配的縣市預報 (模糊匹配)
    const matchingLocation = Object.keys(appState.weatherForecasts).find(key => 
        locationName.includes(key.replace(/[臺台]/g, '台')) // 容忍 "臺" 與 "台" 的差異
    );

    if (!matchingLocation) return '';

    const forecasts = appState.weatherForecasts[matchingLocation];
    const targetDate = appState.travelDate;

    // 找到選定日期/時段的預報
    const targetForecast = forecasts.find(f => 
        targetDate >= f.startTime.split(' ')[0] && targetDate <= f.endTime.split(' ')[0]
    );

    if (!targetForecast) return '';

    // 根據天氣描述選擇圖標和樣式
    let weatherClass = 'weather-tag-clear';
    let icon = '☀️';
    if (targetForecast.weatherDesc.includes('雨')) {
        weatherClass = 'weather-tag-rain';
        icon = '🌧️';
    } else if (targetForecast.weatherDesc.includes('陰') || targetForecast.weatherDesc.includes('多雲')) {
        weatherClass = 'weather-tag-cloudy';
        icon = '☁️';
    }

    return `
        <div class="weather-tag ${weatherClass}">
            ${icon} ${targetForecast.weatherDesc}
            <span style="font-size: 0.9em; margin-left: 5px;">${targetForecast.minT}°C ~ ${targetForecast.maxT}°C</span>
        </div>
    `;
}
...
function initializeSortable() {
    const sortableElement = document.getElementById('suggestionContent');
    if (sortableElement && window.Sortable) {
        new Sortable(sortableElement, {
            ...
            onEnd: function(evt) {
                ...
                const newOrder = items.map(item => item.textContent.trim());
                appState.currentItineraryLocations = newOrder;
                
                // 提示使用者重新生成交通建議
                if (confirm('行程順序已更新，是否重新生成交通建議？')) {
